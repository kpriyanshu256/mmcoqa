...
#        image_rep=self.image_encoder(image_inputs) #(batch_size * num_blocks, hidden_size)
        ...
        if start_positions is not None and end_positions is not None and retrieval_label is not None:
            ...
            logger.info(f'Start End Pos {start_positions.shape} {end_positions.shape}')

            start_positions = start_positions.squeeze(-1).max(dim=1).values
            end_positions = end_positions.squeeze(-1).max(dim=1).values
            
            logger.info(f'Start Pos {start_positions}')
            logger.info(f'End Pos {end_positions}')
            ...
            logger.info(f'Clamp Start Pos {start_positions}')
            logger.info(f'Clamp End Pos {end_positions}')

            logger.info(f'Clamp Start Pos {start_positions.shape}')
            logger.info(f'Clamp End Pos {end_positions.shape}')

            logger.info(f'Clamp Start Logits {start_logits.shape}')
            logger.info(f'Clamp End Logits {end_logits.shape}')
...
class BertForOrconvqaGlobal_v2(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, 
        ...
        if start_positions is not None and end_positions is not None and retrieval_label is not None:
            ...
            # logger.info(f'SL {start_logits.shape}')
            # logger.info(f'EL {end_logits.shape}')



            qa_loss_fct = CrossEntropyLoss(ignore_index=ignored_index, reduction='none')
