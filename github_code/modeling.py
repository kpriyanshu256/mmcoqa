...
from transformers import BertModel, BertPreTrainedModel, AlbertModel, AutoModel, AutoConfig
...
class BertForOrconvqaGlobal(nn.Module):
    r"""
    ...
    def __init__(self, config):
        super(BertForOrconvqaGlobal, self).__init__()

        self.config = config
        self.num_qa_labels = config.num_qa_labels

        self.bert = AutoModel.from_pretrained(config.model_name) # BertModel(config)
        ...
        self.query_encoder = AutoModel.from_pretrained(config.model_name) # BertModel(config)
...
class AlbertForRetrieverOnlyPositivePassage(nn.Module):
    r"""
    ...
    def __init__(self, config, model_path):
        ...
        self.query_encoder = AutoModel.from_config(config) # AlbertModel(config)
        self.query_proj = nn.Linear(config.hidden_size, config.proj_size)
        
        self.passage_encoder = AutoModel.from_config(config) # AlbertModel(config)
        self.passage_proj = nn.Linear(config.hidden_size, config.proj_size)
        self.proj_size = config.proj_size
