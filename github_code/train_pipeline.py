...
from transformers import WEIGHTS_NAME, BertConfig, BertTokenizer, AlbertConfig, AlbertTokenizer, AutoConfig
...
retriever_config = AutoConfig.from_pretrained(args.retrieve_checkpoint)

# load pretrained retriever
retriever_tokenizer = retriever_tokenizer_class.from_pretrained(args.retrieve_tokenizer_dir)
retriever_model = retriever_model_class.from_pretrained(args.retrieve_checkpoint, retriever_config)
...
reader_config.model_name = args.reader_config_name if args.reader_config_name else args.reader_model_name_or_path
...
reader_tokenizer = reader_tokenizer_class.from_pretrained(args.reader_tokenizer_name if args.reader_tokenizer_name else args.reader_model_name_or_path,
                                                          ...
                                                          cache_dir=args.reader_cache_dir if args.reader_cache_dir else None)
reader_model = reader_model_class(reader_config)
...
#             logging.WARN)  # Reduce model loading logs
    ...
    for checkpoint in checkpoints:
        ...
        model = Pipeline()
        model.retriever = retriever_model_class.from_pretrained(args.retrieve_checkpoint, retriever_config)
