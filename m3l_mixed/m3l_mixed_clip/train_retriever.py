...
from modeling import AlbertForRetrieverOnlyPositivePassage, JinaForRetrieverOnlyPositivePassage
...
MODEL_CLASSES = {
    ...
    'jina': (JinaBertConfig, JinaForRetrieverOnlyPositivePassage, AutoTokenizer),
}
...
def gen_passage_rep(args, model, tokenizer):
    ...
    logger.info(f"{type(tokenizer)} {tokenizer}")
    DatasetClass = GenPassageRepDataset
    dataset = DatasetClass( args.gen_passage_rep_input,
                           args.load_small, 
                           tokenizer,
...
if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    #     final_checkpoint_output_dir, force_download=True, trust_remote_code=True)
