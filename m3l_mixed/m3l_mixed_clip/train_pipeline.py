...
from modeling import Pipeline, Reader, Retriever, AlbertForRetrieverOnlyPositivePassage, JinaForRetrieverOnlyPositivePassage
from scorer import quac_eval
from modeling_jina import JinaBertConfig
...
def train(args, train_dataset, model, retriever_tokenizer, reader_tokenizer):
    ...
    for _ in trange(int(args.num_train_epochs), desc='Training'):
        ...
        for step, batch in enumerate(pbar):
            ...
            mbatch_dl = DataLoader(mbatch_ds, batch_size=4, num_workers=4)
            ...
            for mini_batch in tqdm(mbatch_dl, desc="Mini-Batch", leave=False, disable=True):
                ...
                logger.info(f'Batch {inputs["input_ids"].shape}')
...
def evaluate(args, model, retriever_tokenizer, reader_tokenizer, prefix=""):
    ...
    eval_dataloader = DataLoader(dataset, batch_size=args.eval_batch_size, num_workers=0)
    ...
    for b_idx, batch in tqdm(enumerate(eval_dataloader), desc="Evaluating", total=len(eval_dataloader)):
        ...
        mbatch_dl = DataLoader(mbatch_ds, batch_size=32, num_workers=0)
...
if args.model_type == 'albert':
    retriever_config_class, retriever_model_class, retriever_tokenizer_class =\
                AlbertConfig, AlbertForRetrieverOnlyPositivePassage, AlbertTokenizer
else:
    retriever_config_class, retriever_model_class, retriever_tokenizer_class =\
                (JinaBertConfig, JinaForRetrieverOnlyPositivePassage, AutoTokenizer)
...
if args.do_test:    
    ...
    model.retriever = retriever_model_class.from_pretrained(os.path.join(best_checkpoint, 'retriever'), force_download=True)
    model.retriever.passage_encoder = None
    model.retriever.passage_proj = None
    model.retriever.image_encoder = None
    model.retriever.image_proj = None

    model.reader = Reader.from_pretrained(args.reader_model_name_or_path, os.path.join(best_checkpoint, 'reader'), reader_config)
