...
def train(args, train_dataset, model, retriever_tokenizer, reader_tokenizer):
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=args.train_batch_size, num_workers=args.num_workers)
...
if args.do_eval and args.local_rank in [-1, 0]:
    ...
    if args.eval_all_checkpoints:
        checkpoints = sorted(list(os.path.dirname(os.path.dirname(c)) for c in
                                      glob.glob(args.output_dir + '/*/retriever/' + WEIGHTS_NAME, recursive=False)))[1:]
#         logging.getLogger("transformers.modeling_utils").setLevel(
#             logging.WARN)  # Reduce model loading logs

    # checkpoints = ['/data/tir/projects/tir7/user_data/priyansk/m3l_fix_sample_5_v1/checkpoint-10500']
