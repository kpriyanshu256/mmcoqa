...
def evaluate(args, model, retriever_tokenizer, reader_tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        logger.info(f"Q {question_texts}")
        logger.info(f"P {len(passages_for_reader[0])} {len(pids_for_reader[0])}")
        sys.exit(0)
    ...
    all_predictions = write_predictions(reader_tokenizer, examples, features, all_results, args.n_best_size,
