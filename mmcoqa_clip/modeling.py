...
class BertForOrconvqaGlobal(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.image_encoder_fc = nn.Linear(512, config.hidden_size)
        ...
        self.image_encoder = prepare_model(chkpt_dir, 'mae_vit_large_patch16')
        self.image_encoder_fc = nn.Linear(197*1024, config.hidden_size)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, 
        ...
        image_inputs=image_input.view(-1,3,*self.im_size).float()    

        image_rep= self.image_encoder_fc(self.image_encoder.forward_encoder(image_inputs, 0)[0].reshape(-1, 197*1024)) #(batch_size * num_blocks, hidden_size)
        # image_rep= self.image_encoder_fc(self.image_encoder(image_inputs.type(torch.FloatTensor).cuda())) #(batch_size * num_blocks, hidden_size)
...
class AlbertForRetrieverOnlyPositivePassage(AlbertPreTrainedModel):
    ...
    def forward(self, query_input_ids=None, query_attention_mask=None, query_token_type_ids=None, 
        ...
        if passage_input_ids is not None:
            ...
            #####################encode an image
            # image_outputs = self.image_encoder_fc(self.image_encoder(image_input))
            image_outputs = self.image_encoder_fc(self.image_encoder.forward_encoder(image_input, 0)[0].reshape(-1, 197*1024))
